...
class Disk:
    ...
    def __init__(self, alpha=1e-100, m_star_solarmasses=1, gas_dep=1., sol_gas_ratio=0.01,
    	         sig_p_in=0, temp_in=0):
        ...
        ### Set disk parameters ###
    	self.sig_gas = fn.surf_dens(a_core)/gas_dep # Calculates gas surface density
        ...
        self.v_rel_i = self.v_core_gas # Sets initial velocity of core relative to gas
        ...
        self.v_gas_tot = np.sqrt(self.v_gas_turb**2 + (self.v_gas_lam - self.v_core)**2) # Total velocity of gas
...
class Core(Disk):
	...
	def __init__(self, a_core_au=1, m_core_earthmasses=1, alpha=1e-100, m_star_solarmasses=1, gas_dep=1.,
	             sol_gas_ratio=0.01, rho_core=2., sig_p_in=0, temp_in=0, r_shear_off=0, extend_rh=1, alpha_z=0, h_mod=1):
		""" List of old names -> new names:
		    ...
		    area -> area_acc
		    time -> t_acc """
		super().__init__(alpha, m_star_solarmasses, gas_dep, sol_gas_ratio, sig_p_in, temp_in)
		...
		self.r_core = ((3 * m_core)/(4 * np.pi * rho_core))**(1/3) # Calculates radius of core
    ...
    def t_stop(s, rho_obj=2.):
    	...
    	if s > 9./4 * self.mfp: # Solve iteratively if we're in the fluid regime
    	    ...
    	    while np.abs(delta) > .001:
                ...
                if stl > 10:
                	v_new_turb = v_gas_turb * np.sqrt(1 - (1 + stl)**-1)
                else:
                	v_new_turb = fn.v_pg(v_gas_turb, stl, self.re_f)
        ...
        self.t_eddy = (self.om * (1 + (self.v_lam/self.v_gas_tub)**2)**.5)**-1 # Eddy crossing time
    ...
    def r_wish(s, rho_obj=2.):
    	""" Calculates wind-shearing radius, given particle size s. """
    	v_cap = self.v_gas_tot # Set relevant velocity for orbit maintain HUH?
    	...
    	self.delta_a = np.abs(f_drag_core/m_obj - f_drag_core/self.m_core) # Differential acceleration between core and object

    	self.r_ws = fn.wish_radius(self.delta_a, self.m_core, self.m_obj) # Wind shearing radius
    	return self.r_ws


    def r_shear(s, rho_obj=2.):
    ...
    def set_velocities():
    	...
    	self.v_obj_phi = np.abs(self.v_phi_lam + self.v_gas_lam) # Object velocity relative to Keplerian
    	self.v_lam_iner = np.sqrt((self.v_obj_phi - self.v_core)**2 + self.v_r_lam**2) # Laminar, relative to inertial frame
    	self.v_obj_turb = np.sqrt(self.v_gas_turb**2 - self.obj_gas_turb**2) # WHICH FRAME IS THIS?
    	self.v_obj = np.sqrt(self.v_obj_turb**2 + self.v_lam_iner**2) # Total body velocity relative to Keplerian

    	self.v_obj_core = self.v_obj # Velocity of object relative to core [WILL CHANGE FROM RELATIVE TO KEP]
    ...
    def scale_heights():
    	...
    	if not(alpha_z): # Checks if vertical turbulence is different from other
    	...
    	self.area_acc = 4 * self.r_acc * H_acc # Area over which objects are accreted
