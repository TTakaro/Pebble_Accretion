...
class RVSystem(RVPlanet):
    ...
    def log_like(self,jitter=0,sol_type = 'int'):
        ...
        for i in range(rng):
            ...
            err_tot = np.sqrt(errs**2.+jitter**2.)
    ...
    def calc_chi2(self,jitter=0,reduced=False,non_planar = False,rms=False):
        ...
        for i in range(rng):
            if flat:
                ...
                errs = self.errs[i]
                vels_model = self.vels_model[i]
            chi_2 += np.sum(((vels-self.offsets[i]-vels_model)**2.)/(errs**2+jitter**2.))
            points += len(vels)
        if reduced:
            if jitter:
                jit_num = 1
